...
function enterCloudWorld(me, nopause) {
  ...
  var movement = Global.setInterval(function() {
    // Vine moving up
    if(me.attached.top <= screentop) {
      ...
      movement = Global.setInterval(function() {
        // Mario moving up
...
function walkToPipe() {
  ...
  var move = Global.setInterval(function() {
    if(mario.piping) {
...
function intoPipeVert(me, pipe, transport) {
  ...
  var move = Global.setInterval(function() {
    ...
    if(me.top >= pipe.top) {
      clearInterval(move);
      Global.setTimeout(function() { goToTransport(transport); }, 700);
    }
  }, timer);
}
...
function exitPipeVert(me, pipe) {
  ...
  var dy = unitsize / -4, move = Global.setInterval(function() {
...
function BlankMap(map) {
  ...
  map.areas = [ new Area("Overworld", function() {
    Global.setTimeout(refillCanvas, timer + 2);
  }) ];
}
