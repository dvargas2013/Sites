...
BOARD = function (mx, my) {
	this.mx = Math.min(40,Math.max(3,mx || 2));
	this.my = Math.min(20,Math.max(3,my || 2));
	this.matrix = [];
...
window.onload = init;
// TODO initialize with a path cycle
// the initial path is a 2x2 basically
// you must move with that cycle in mind
// the cycle can be edited as follow:
//  	2 blank blocks and 2 cycle blocks such that diagonals are opposite type
//  	uuum it should be simple to see how to add the 2 blocks tbh
//  basically the hard part is knowing where to add blocks to the cycle 
var STATE, AI;
/*
...
STATE.prototype.coolmoves = function() {
	...
	var maxdttail = [0,1,2,3].map(function(i){
		var mm = states[i].board.dttail()
		for (var j=0;j<states[i].clones.length;j++) {
	...
	return me.moves.filter(function(i){ // correct tail moves
		return !!tailg[i];
	}).sort(function(){
	...
	// TODO sometimes gets stuck trying to follow tail when it could take alternate path and still return to tail
		...
		return me.clones[a].snake.distTo(aim) - me.clones[b].snake.distTo(aim);
	}).sort(function (a, b) { // highest of scores
