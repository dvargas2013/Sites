...
function Thing(type) {
  ...
  try { setContextStuff(self, self.spritewidth, self.spriteheight); }
    ...
    Global.setTimeout(function() { setContextStuff(self, self.spritewidth, self.spriteheight); }, 1);
  }
...
function movePirhanaRestart(me) {
  ...
  if(me.counter >= me.countermax && marmid > me.left - unitsizet8 && marmid < me.right + unitsizet8) {
    Global.setTimeout(movePirhanaRestart, 7, me);
    return;
  }
...
function killMario(me, big) {
  ...
  // If the map is normal, or failing that a game over is reached, timeout a reset
    ...
    window.reset = Global.setTimeout(data.lives.amount ? setMap : gameOver, timer * 280);
  }
...
function gameOver() {
  ...
  Global.setTimeout(gameRestart, 7000);
}
...
function attachEmerge(me, solid) {
  me.animate = Global.setInterval(function() {
...
function FlagCollision(me, detector) {
  ...
  var mebot = false,
      ...
      down = Global.setInterval(function() {
        ...
        if(mebot && flagbot) {
          setBottom(me, detector.stone.top, true);
          clearInterval(down);
          Global.setTimeout(function() { FlagOff(me, detector.pole); }, timer * 21);
        }
        refillCanvas();
      }, timer);
}
...
function endLevelPoints(me, detector) {
  ...
  var points = Global.setInterval(function() {
    // 50 for each
    --data.time.amount;
    ...
    if(data.time.amount <= 0)  {
      ...
      Global.setTimeout(function() { endLevelFireworks(me, numfire, detector); }, timer * 49);
    }
  }, timerd2);
}
function endLevelFireworks(me, numfire, detector) {
  ...
  nextfunc = function() { Global.setTimeout(function() { endLevel(); }, nextnum); };
